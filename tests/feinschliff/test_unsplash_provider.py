...
def _reset_stub_warning(monkeypatch):
    ...
    from feinschliff.io.providers import unsplash as unsplash_mod
...
def test_search_with_key_returns_parsed_image_hit():
    ...
    with patch("feinschliff.io.providers.unsplash.urlopen") as mock_urlopen:
...
def test_search_retries_then_gives_up_on_429():
    ...
    with patch("feinschliff.io.providers.unsplash.urlopen", side_effect=err) as mock_urlopen, \
            patch("feinschliff.io.providers.unsplash.time.sleep") as mock_sleep:
...
def test_search_retries_then_gives_up_on_500():
    ...
    with patch("feinschliff.io.providers.unsplash.urlopen", side_effect=err) as mock_urlopen, \
            patch("feinschliff.io.providers.unsplash.time.sleep"):
        hits = p.search("kitchen")
...
def test_search_handles_socket_timeout():
    ...
    with patch(
        "feinschliff.io.providers.unsplash.urlopen",
...
def test_search_retry_then_success():
    ...
    with patch("feinschliff.io.providers.unsplash.urlopen", side_effect=side_effects) as mock_urlopen, \
            patch("feinschliff.io.providers.unsplash.time.sleep"):
        hits = p.search("kitchen")
...
def test_search_handles_malformed_json():
    ...
    with patch("feinschliff.io.providers.unsplash.urlopen", return_value=cm), \
            patch("feinschliff.io.providers.unsplash.time.sleep"):
        hits = p.search("kitchen")
...
def test_unsplash_self_registers_via_package_import(monkeypatch):
    ...
    for mod_name in list(sys.modules):
        if mod_name == "feinschliff.io.providers" or mod_name.startswith("feinschliff.io.providers."):
            sys.modules.pop(mod_name, None)

    import feinschliff.io.providers  # noqa: F401 — side-effect import
