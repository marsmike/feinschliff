    ...
    for el in doc.get("elements", []):
        ...
        if want_w > p_box.w - slack_w + _OVERFLOW_TOLERANCE_PX:
            out.append(Defect(
                ...
                extra={"axis": "width", "element_id": el.get("id"),
        ...
        if want_h > p_box.h - slack_h + _OVERFLOW_TOLERANCE_PX:
            out.append(Defect(
                ...
                extra={"axis": "height", "element_id": el.get("id"),
                      "container_id": parent_id},
            ))
    return out
    ...
    for i, a in enumerate(shapes):
        ...
        for b in shapes[i + 1:]:
            ...
            out.append(Defect(
                ...
                extra={"a_id": a.get("id"), "b_id": b.get("id")},
            ))
    return out
        ...
        for r in rects:
            ...
            if _intersects(arrow_box, rb, tolerance=0):
                out.append(Defect(
                    ...
                    extra={"arrow_id": el.get("id"), "shape_id": r.get("id")},
                ))
    return out
...
def validate_svg_structure(svg_text: str) -> list[Defect]:
    ...
    try:
        ...
        return [Defect(
            kind=DefectKind.DIAGRAM_INVALID_FILE,
            severity=Severity.ERROR,
...
def validate_diagram_file(path: Path) -> list[Defect]:
    ...
    if ext == ".svg":
        ...
        if any(d.severity == Severity.ERROR for d in defects):
            return defects
