...
These examples target the `svg-infographic-full` layout (full-slide,
1720×720 slot, virtual 6880×2880 canvas). They demonstrate the extended
...
Source files live in `feinschliff/skills/svg/examples/`.
...
**Teaches:** how the new primitives compose. A deck slide showing this
...
## When not to use the full layout

The `svg-infographic-full` layout is for dense compositions. If the
chart has fewer than 8 elements and a single visual idea, the narrow
`svg-infographic` is faster to read. The 4× virtual canvas isn't
"better quality" — it's "more room for complexity."
...
When in doubt, draft on the narrow layout first; promote to full only if
content visibly cramps.
