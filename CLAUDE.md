...
(`tests/feinschliff/fixtures/verify_quality/clean-deck.pptx`), and the
...
## Tests live at the repo root, never inside plugins

All Python tests live in `tests/<package>/` at the repo root (e.g.
`tests/feinschliff/`, `tests/feinbild/`), NOT inside the package
directories. Claude Code plugin installs copy the entire plugin source
directory verbatim — there is no ignore/exclude mechanism — so anything
under `<plugin>/tests/` would ship to every user's plugin cache. Run them
from the repo root: `uv run pytest tests/<package>`. Shared layout content
fixtures consumed by the brand-atlas/preview scripts live in
`tests/feinschliff/fixtures/layouts/`. The exception is per-brand fixture
overrides (`brands/<brand>/tests/fixtures/layouts/<id>.yaml`) — that path
is a brand-pack convention also used by external packs, so it stays inside
the brand directories.
