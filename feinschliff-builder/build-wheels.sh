#!/usr/bin/env bash
# Rebuild feinschliff-builder/wheels/ — the offline wheelhouse the bin/ launcher installs.
...
uv build --wheel --out-dir "$BUILD" "$ROOT/feinschliff"   # feinschliff
uv build --wheel --out-dir "$BUILD" "$ROOT/feinschmiede"   # feinschmiede
cp "$BUILD"/feinschliff_builder-*.whl "$BUILD"/feinschliff-*.whl "$BUILD"/feinschmiede-*.whl "$STAGE"/
