...
Reports, per layout and in total:
  ...
  * content-sized native pictures — raster pics carried as fixed chrome
    although they exceed mark size (min dimension > --max-mark-px): photos,
    tile thumbnails and illustration strips that should be picture slots;
...
content-sized native pics), 1 otherwise — wire it into CI or run it as the
...
def register(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--max-mark-px", type=int, default=64,
                        help="Raster native pics with min(w, h) above this are "
                             "flagged as unslotted content images (default 64)")
    ...
    for path in sorted(layouts_dir.glob("*.slide.dsl")):
        ...
        for line in body.splitlines():
            ...
            if min(w, h) > args.max_mark_px:
                content_pics.append(f"{line.split()[1]} {w:.0f}x{h:.0f}px")
